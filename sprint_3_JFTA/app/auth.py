    ...
    try:
        ...
        if request.method == 'POST':    
            ...
            if not utils.isUsernameValid(username):
                error = "El nombre de usuario debe ser alfanumérico ó con '.','_','-'"
                flash(error)
                return render_template('auth/register.html')
            ...
            if (not utils.isPasswordValid(password)):
                error = 'La contraseña debe contener al menos una letra minúscula, una letra mayúscula y un número, y no podrá ser menor a 8 caracteres'
                flash(error)
                return render_template('auth/register.html')
            ...
            content = 'Hola, para activar la cuenta por favor acceder al siguiente enlace ' + flask.url_for('auth.activate', _external=True) + '?auth=' + number
            
            send_email(credentials, receiver=email, subject='Activate your account', message=content)
            
            flash('Por favor verifique en el email registrado para activar la cuenta')
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            if not password1:
                flash('Se requiere confirmación de la contraseña')
                return render_template('auth/change.html', number=authid)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                content = 'Hola, para cambiar su contraseña por favor acceda al siguiente enlace ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
                
                send_email(credentials, receiver=email, subject='New Password', message=content)
                
                flash('Por favor revise el email registrado')
            else:
                error = 'Email no está registrado'
                flash(error)            
...
def login():
    try:
        ...
        if request.method == 'POST':
            ...
            if not username:
                error = 'Se requiere llenar el campo del nombre de usuario'
                flash(error)
                return render_template('auth/login.html')

            if not password:
                error = 'Se requiere llenar el campo de la contraseña'
                flash(error)
                return render_template('auth/login.html')
